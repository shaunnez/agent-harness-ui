...
gen=Path('/Users/shaun/.codex/generated_images/01a0749d-4c28-78f1-ba94-00501ef2ade3/exec-db8d11ec-a01c-4b06-a540-9a66ae8cf03e.png');shutil.copy2(gen,R/'source/delivery-inspection-master-r1.png');m=Image.open(gen).convert('RGB');a=np.array(m).astype(float);ex=a[:,:,1]-np.maximum(a[:,:,0],a[:,:,2]);alpha=1-np.clip((ex-20)/130,0,1);a[:,:,1]=np.where(ex>20,np.minimum(a[:,:,1],np.maximum(a[:,:,0],a[:,:,2])+10),a[:,:,1]);ar=np.dstack((a,alpha*255)).clip(0,255).astype('uint8');ar[ar[:,:,3]==0,:3]=0;cut=Image.fromarray(ar);cut.save(R/'source/delivery-inspection-alpha-master-r1.png');box=cut.getbbox();s=320/(box[2]-box[0]);size=(320,round((box[3]-box[1])*s));offset=(96,430-size[1]);out=Image.new('RGBA',(512,512));out.alpha_composite(cut.crop(box).resize(size,Image.Resampling.LANCZOS),offset);ar=np.array(out);ex=ar[:,:,1].astype(float)-np.maximum(ar[:,:,0],ar[:,:,2]);ar[:,:,1]=np.where(ex>20,np.minimum(ar[:,:,1],np.maximum(ar[:,:,0],ar[:,:,2]).astype(int)+10),ar[:,:,1]);ar[ar[:,:,3]==0,:3]=0;out=Image.fromarray(ar);e=R/'exports/station-delivery-inspection-r1.png';out.save(e)
def p(v):return [round(offset[0]+(v[0]-box[0])*s,4),round(offset[1]+(v[1]-box[1])*s,4)]
pts={k:p(v) for k,v in {'frontFoot':[883,809],'leftFoot':[439,635],'rightFoot':[1326,657],'frontSurface':[883,574],'frontToolPort':[496,563],'screenCenter':[576,310],'inspectionCase':[858,388]}.items()};anchor=[v/2 for v in pts['frontFoot']];fp=[pts['leftFoot'],p([882,398]),pts['rightFoot'],pts['frontFoot']];height=(pts['frontFoot'][1]-pts['frontSurface'][1])/2
...
(R/'qa/QA.md').write_text('''# Delivery inspection station A2 r1
