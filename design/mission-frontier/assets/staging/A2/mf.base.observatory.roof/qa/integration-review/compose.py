...
R=Path(__file__).resolve().parent;repo=next(p for p in R.parents if (p/'src/frontier/world/scene.ts').exists());pub=repo/'public/frontier';mp=pub/'assets/manifest.json';m=json.loads(mp.read_text());E={x['id']:x for x in m['assets']};cache={}
