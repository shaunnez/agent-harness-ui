...
gen=Path('/Users/shaun/.codex/generated_images/01a0749d-4c28-78f1-ba94-00501ef2ade3/exec-6522454f-a646-42f6-bcc5-f178d0c8c900.png');shutil.copy2(gen,R/'source/survey-master-r1.png');m=Image.open(gen).convert('RGB');a=np.array(m).astype(float);ex=a[:,:,1]-np.maximum(a[:,:,0],a[:,:,2]);alpha=1-np.clip((ex-20)/130,0,1);a[:,:,1]=np.where(ex>20,np.minimum(a[:,:,1],np.maximum(a[:,:,0],a[:,:,2])+10),a[:,:,1]);ar=np.dstack((a,alpha*255)).clip(0,255).astype('uint8');ar[ar[:,:,3]==0,:3]=0;cut=Image.fromarray(ar);cut.save(R/'source/survey-alpha-master-r1.png');box=cut.getbbox();s=320/(box[2]-box[0]);size=(320,round((box[3]-box[1])*s));offset=(96,430-size[1]);out=Image.new('RGBA',(512,512));out.alpha_composite(cut.crop(box).resize(size,Image.Resampling.LANCZOS),offset);ar=np.array(out);ex=ar[:,:,1].astype(float)-np.maximum(ar[:,:,0],ar[:,:,2]);ar[:,:,1]=np.where(ex>20,np.minimum(ar[:,:,1],np.maximum(ar[:,:,0],ar[:,:,2]).astype(int)+10),ar[:,:,1]);ar[ar[:,:,3]==0,:3]=0;out=Image.fromarray(ar);e=R/'exports/station-survey-r1.png';out.save(e)
def p(v):return [round(offset[0]+(v[0]-box[0])*s,4),round(offset[1]+(v[1]-box[1])*s,4)]
pts={k:p(v) for k,v in {'frontFoot':[839,912],'leftFoot':[347,690],'rightFoot':[1207,701],'frontSurface':[839,677],'frontToolPort':[428,631],'screenCenter':[533,398],'receiver':[1049,216]}.items()};anchor=[v/2 for v in pts['frontFoot']];fp=[pts['leftFoot'],p([642,466]),pts['rightFoot'],pts['frontFoot']];height=(pts['frontFoot'][1]-pts['frontSurface'][1])/2
