...
gen=Path('/Users/shaun/.codex/generated_images/01a0749d-4c28-78f1-ba94-00501ef2ade3/exec-9140f386-f87b-4184-a1fc-2e0ddfae7c6a.png');shutil.copy2(gen,R/'source/intake-master-r1.png');m=Image.open(gen).convert('RGB');a=np.array(m).astype(float);ex=a[:,:,1]-np.maximum(a[:,:,0],a[:,:,2]);alpha=1-np.clip((ex-20)/130,0,1);a[:,:,1]=np.where(ex>20,np.minimum(a[:,:,1],np.maximum(a[:,:,0],a[:,:,2])+10),a[:,:,1]);ar=np.dstack((a,alpha*255)).clip(0,255).astype('uint8');ar[ar[:,:,3]==0,:3]=0;cut=Image.fromarray(ar);cut.save(R/'source/intake-alpha-master-r1.png');box=cut.getbbox();s=320/(box[2]-box[0]);size=(320,round((box[3]-box[1])*s));offset=(96,430-size[1]);out=Image.new('RGBA',(512,512));out.alpha_composite(cut.crop(box).resize(size,Image.Resampling.LANCZOS),offset);ar=np.array(out);ex=ar[:,:,1].astype(float)-np.maximum(ar[:,:,0],ar[:,:,2]);ar[:,:,1]=np.where(ex>20,np.minimum(ar[:,:,1],np.maximum(ar[:,:,0],ar[:,:,2]).astype(int)+10),ar[:,:,1]);ar[ar[:,:,3]==0,:3]=0;out=Image.fromarray(ar);e=R/'exports/station-intake-r1.png';out.save(e)
def p(v):return [round(offset[0]+(v[0]-box[0])*s,4),round(offset[1]+(v[1]-box[1])*s,4)]
pts={k:p(v) for k,v in {'frontFoot':[731,1008],'leftFoot':[193,779],'rightFoot':[1082,799],'frontSurface':[731,759],'frontToolPort':[282,703],'screenCenter':[577,550],'receiver':[935,241]}.items()};anchor=[v/2 for v in pts['frontFoot']];fp=[pts['leftFoot'],p([478,559]),pts['rightFoot'],pts['frontFoot']];height=(pts['frontFoot'][1]-pts['frontSurface'][1])/2
...
for row,factor,wh,y0 in [(0,1.275,74,30),(1,2.52,200,410)]:
 for col,station in enumerate([fabric,communications,out]):
  ...
  dr=ImageDraw.Draw(qa);dr.text((col*750+35,y0+5),['Fabrication','Communications','Intake'][col]+(' /HQ1.275x, worker74px' if row==0 else ' /detail2.52x, worker200px'),fill='white')
qa.convert('RGB').save(R/'qa/calibrated-scale-comparison-r1.png')
entry={'id':'mf.station.intake','revision':1,'status':'pending-coordinator-A2-review','file':str(e.relative_to(R)),'sourceSize':[512,512],'logicalSize':[256,256],'coordinateUnits':'logical-pixels-untrimmed','groundAnchor':anchor,'footprint':[[v/2 for v in q] for q in fp],'footprintUse':'Approximate support footprint; rear point is occluded estimate.','layer':'station','occlusionClass':'solid-station','sockets':{k:[v/2 for v in pts[k]] for k in ['frontToolPort','screenCenter','receiver']},'sha256':sha(e),'alpha':'straight RGBA','geometry':'geometry.json','qa':['qa/QA.md','qa/alpha-backgrounds-r1.png','qa/socket-registration-r1.png','qa/calibrated-scale-comparison-r1.png'],'limits':['Static unlit glass briefing panel and beacon; illumination belongs to runtime.','No workers, floor, state labels or animated parts baked in.','A2 integrated scene review pending.']};write('entry.json',entry)
refs=[Path('/Users/shaun/.codex/worktrees/7237/agent-harness-ui/design/mission-frontier/screens/project-base-attention-v1.1.png'),A0/'mf.station.fabrication/exports/station-fabrication-r1.png',R.parents[1]/'A1/mf.station.communications/exports/station-communications-r1.png'];write('provenance.json',{'assetId':entry['id'],'revision':1,'generator':'Built-in ImageGen','sourceId':'exec-9140f386-f87b-4184-a1fc-2e0ddfae7c6a','sourceSha256':sha(R/'source/intake-master-r1.png'),'initialSourceId':'exec-494dfb1f-4991-4f74-b3ca-23bfa63b5186','initialSourceSha256':sha(gen.parent/'exec-494dfb1f-4991-4f74-b3ca-23bfa63b5186.png'),'prompt':(R/'source/generation-prompt.txt').read_text(),'references':[{'path':str(v),'sha256':sha(v)} for v in refs],'correctiveGenerations':1,'correctionPrompt':(R/'source/correction-prompt.txt').read_text(),'transforms':['Green key and spill cleanup','Crop subject and uniformly fit320source pixels wide','Lanczos resize and register bottom at430source on512canvas','QA-only comparison at requested consumer factors'],'exportSha256':sha(e),'exportBytes':e.stat().st_size,'rightsNote':'Supplied/generated project references only; provenance is not legal clearance.'})
...
(R/'qa/QA.md').write_text('''# Intake station A2 r1

Distinct broad glass briefing panel and short unlit intake column with physical slot. Ceramic/slate materials and warm light match fabrication. No worker/floor/wall/task text or status symbols; glass is reflective with no semantic illumination. Initial RGB checkerboard output retained; one ImageGen correction replaced it with flat green for keying.
...
''')
shutil.copy2(gen.parent/'exec-494dfb1f-4991-4f74-b3ca-23bfa63b5186.png',R/'source/intake-initial-checkerboard.png')
