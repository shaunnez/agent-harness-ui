...
GEN=Path('/Users/shaun/.codex/generated_images/01a0749d-4c28-78f1-ba94-00501ef2ade3/exec-14af2a67-be75-4fc0-9b03-942745d8f2d2.png')
MASTER=ROOT/'source/base-standard-back-master-r1.png'
...
cutout=Image.fromarray(rgba);cutout.save(ROOT/'source/base-standard-back-alpha-master-r1.png')
...
source_center=767.5;sx=1408/1423;source_ground_y=270;source_slope=328/679
target_center=768;target_inner_ground_y=164;height_scale=.78
out=Image.new('RGBA',(1536,1024));coefficients={}
for side,sign,region in [('left',-1,(0,0,768,1024)),('right',1,(768,0,1536,1024))]:
    ...
    warped=cutout.transform((1536,1024),Image.Transform.AFFINE,cf,Image.Resampling.BICUBIC)
...
export=ROOT/'exports/base-standard-back-r1.png';out.save(export)
floorpath=Path('/Users/shaun/.codex/worktrees/7237/agent-harness-ui/design/mission-frontier/assets/staging/A0/mf.base.standard.floor/exports/base-standard-floor-r1.png')
floor=Image.open(floorpath).convert('RGBA');assembly=floor.copy();assembly.alpha_composite(out)
assembly.save(ROOT/'qa/floor-back-composite-r1.png')
...
sheet=Image.new('RGB',(1536,376),'#22303a');d=ImageDraw.Draw(sheet)
for i,(name,color) in enumerate([('Dark','#071923'),('Light','#f0f3f5'),('Magenta','#d000cc')]):
    sheet.paste(bg(out,color,(512,341)),(i*512,28));d.text((i*512+12,8),name+' / walls only',fill='white')
sheet.save(ROOT/'qa/alpha-backgrounds-r1.png');bg(assembly,'#071923',(768,512)).save(ROOT/'qa/hq-composite-logical-scale-r1.png')
geo={'canvas':[1536,1024],'logicalCanvas':[768,512],'commonAnchorSource':[768,844],'commonAnchorLogical':[384,422],'floorSurfaceCornersSource':[[768,140],[1472,492],[768,844],[64,492]],'sourceInnerGroundPath':[[88.5,598],[767.5,270],[1446.5,598]],'registeredInnerGroundPath':[[round(768+(88.5-767.5)*sx,4),round(164+abs((88.5-767.5)*sx)*.5,4)],[768,164],[round(768+(1446.5-767.5)*sx,4),round(164+abs((1446.5-767.5)*sx)*.5,4)]],'sourceWidthControl':[56,1479],'targetWidthControl':[64,1472],'sourceTopBackApprox':[767.5,129],'registeredTopBackApprox':[768,round(164+.78*(129-270),4)],'heightScale':height_scale,'horizontalScale':sx,'inverseAffineCoefficients':coefficients,'alphaBounds':out.getbbox(),'surfaceOffsetConvention':'Inner visible wall feet are 24 export pixels toward the floor interior relative to rear perimeter line. Outer wall structure/foot reaches the perimeter. Common anchor is inherited from floor, not wall bounding box.','measurementMethod':'Manual source foot and top landmarks within approximately 3 pixels, then two joined affine face registrations split at target x768. New exports share target floor landmarks; no new floor pixels synthesized.'}
(ROOT/'geometry.json').write_text(json.dumps(geo,indent=2)+'\n')
registration=bg(assembly,'#071923',(1536,1024));d=ImageDraw.Draw(registration)
rear=[(64,492),(768,140),(1472,492)];d.line(rear,fill='#ffb64c',width=2);d.line([tuple(p) for p in geo['registeredInnerGroundPath']],fill='#61d7ff',width=2);d.text((20,20),'Orange: floor rear perimeter. Cyan: measured wall inner-contact path.',fill='white');registration.save(ROOT/'qa/registration-composite-r1.png')
stats={'mode':out.mode,'size':list(out.size),'alphaRange':out.getchannel('A').getextrema(),'transparentPixels':int((arr[:,:,3]==0).sum()),'partialAlphaPixels':int(((arr[:,:,3]>0)&(arr[:,:,3]<255)).sum()),'borderNontransparent':int((arr[[0,-1],:,3]>0).sum()+(arr[:,[0,-1],3]>0).sum()),'interiorFloorRegionNontransparent':int((arr[560:800,450:1050,3]>0).sum()),'greenDominantVisiblePixels':int(((arr[:,:,1].astype(float)-np.maximum(arr[:,:,0],arr[:,:,2])>30)&(arr[:,:,3]>16)).sum()),'pngBytes':export.stat().st_size,'decodedRgbaBytes':1536*1024*4}
