...
gen=Path('/Users/shaun/.codex/generated_images/01a0749d-4c28-78f1-ba94-00501ef2ade3/exec-4631d35e-baff-45ed-9d55-6042e1d8ae66.png');shutil.copy2(gen,R/'source/inspection-master-r1.png');m=Image.open(gen).convert('RGB');a=np.array(m).astype(float);ex=a[:,:,1]-np.maximum(a[:,:,0],a[:,:,2]);alpha=1-np.clip((ex-20)/130,0,1);a[:,:,1]=np.where(ex>20,np.minimum(a[:,:,1],np.maximum(a[:,:,0],a[:,:,2])+10),a[:,:,1]);ar=np.dstack((a,alpha*255)).clip(0,255).astype('uint8');ar[ar[:,:,3]==0,:3]=0;cut=Image.fromarray(ar);cut.save(R/'source/inspection-alpha-master-r1.png');box=cut.getbbox();s=320/(box[2]-box[0]);size=(320,round((box[3]-box[1])*s));offset=(96,430-size[1]);out=Image.new('RGBA',(512,512));out.alpha_composite(cut.crop(box).resize(size,Image.Resampling.LANCZOS),offset);ar=np.array(out);ex=ar[:,:,1].astype(float)-np.maximum(ar[:,:,0],ar[:,:,2]);ar[:,:,1]=np.where(ex>20,np.minimum(ar[:,:,1],np.maximum(ar[:,:,0],ar[:,:,2]).astype(int)+10),ar[:,:,1]);ar[ar[:,:,3]==0,:3]=0;out=Image.fromarray(ar);e=R/'exports/station-inspection-r1.png';out.save(e)
def p(v):return [round(offset[0]+(v[0]-box[0])*s,4),round(offset[1]+(v[1]-box[1])*s,4)]
pts={k:p(v) for k,v in {'frontFoot':[623,1114],'leftFoot':[194,843],'rightFoot':[1104,846],'frontSurface':[623,847],'frontToolPort':[211,712],'screenCenter':[566,236],'scanCenter':[628,582],'sensorHead':[805,450]}.items()};anchor=[v/2 for v in pts['frontFoot']];fp=[pts['leftFoot'],p([634,573]),pts['rightFoot'],pts['frontFoot']];height=(pts['frontFoot'][1]-pts['frontSurface'][1])/2
...
qa=Image.new('RGBA',(1500,1040),'#071923');dr=ImageDraw.Draw(qa)
for row,factor,wh,y0 in [(0,1.275,74,30),(1,2.52,200,410)]:
 for col,station in enumerate([fabric,out]):
  ...
  dr=ImageDraw.Draw(qa);dr.text((col*750+35,y0+5),('Fabrication' if col==0 else 'Inspection')+(' /HQ1.275x, worker74px' if row==0 else ' /detail2.52x, worker200px'),fill='white')
qa.convert('RGB').save(R/'qa/calibrated-scale-comparison-r1.png')
entry={'id':'mf.station.inspection','revision':1,'status':'pending-coordinator-A1-review','file':str(e.relative_to(R)),'sourceSize':[512,512],'logicalSize':[256,256],'coordinateUnits':'logical-pixels-untrimmed','groundAnchor':anchor,'footprint':[[v/2 for v in q] for q in fp],'footprintUse':'Approximate support footprint; rear point is occluded estimate.','layer':'station','occlusionClass':'solid-station','sockets':{k:[v/2 for v in pts[k]] for k in ['frontToolPort','screenCenter','scanCenter','sensorHead']},'sha256':sha(e),'alpha':'straight RGBA','geometry':'geometry.json','qa':['qa/QA.md','qa/alpha-backgrounds-r1.png','qa/socket-registration-r1.png','qa/calibrated-scale-comparison-r1.png'],'limits':['Static neutral powered scan platter/readouts; waiting/finished require no art variant. Sensor arm is scenery for this ID, not an independently articulated part.','No workers, floor, state labels or animated parts baked in.','A1 integrated scene review pending.']};write('entry.json',entry)
refs=[Path('/Users/shaun/.codex/worktrees/7237/agent-harness-ui/design/mission-frontier/screens/project-base-attention-v1.1.png'),A0/'mf.station.fabrication/source/station-fabrication-master-r1.png'];write('provenance.json',{'assetId':entry['id'],'revision':1,'generator':'Built-in ImageGen','sourceId':'exec-4631d35e-baff-45ed-9d55-6042e1d8ae66','sourceSha256':sha(R/'source/inspection-master-r1.png'),'prompt':(R/'source/generation-prompt.txt').read_text(),'references':[{'path':str(v),'sha256':sha(v)} for v in refs],'correctiveGenerations':0,'transforms':['Green key and spill cleanup','Crop subject and uniformly fit320source pixels wide','Lanczos resize and register bottom at430source on512canvas','QA-only comparison at requested consumer factors'],'exportSha256':sha(e),'exportBytes':e.stat().st_size,'rightsNote':'Supplied/generated project references only; provenance is not legal clearance.'})
...
(R/'qa/QA.md').write_text('''# Inspection station A1 r1

Distinct recessed circular scan platter, two readout screens and compact inspection sensor arm, same ceramic/slate materials and warm light as fabrication. No worker/floor/wall/task text or status symbols. Abstract display lines are static powered hardware, not activity evidence.
...
QA compares approved fabrication and independent worker at HQ0.85×1.5=1.275 and detail2.1×1.2=2.52. Worker shown74px/200px. Alpha inspected on dark/light/magenta. Main renderer owns contact, live state, selection and attention. Pending coordinator A1 integration review.
''')
