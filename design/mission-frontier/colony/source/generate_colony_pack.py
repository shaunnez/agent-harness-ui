...
contract = {
  ...
  "colony": {
    ...
    "slots": slots,
    "fillRule": "A project takes the lowest free project slot P1, P2, ... at first sighting (fixture: sorted by createdAt then id). The slot is persisted with the project appearance record and never reassigned while the project exists. Archived projects keep their slot; their base is shown dormant. Beyond P18, continue ring 3 clockwise from phi=90 at 3D.",
    "edges": edges,
  ...
  "hq": {
    ...
    "occupancy": {"socketPitch": SOCKET_PITCH, "minRobotSpacing": ROBOT_CLEAR,
                  "allocation": ["1. room primary sockets in listed order (wall row first, then bench/inner row)",
                                 ...
                                 "4. overflow lane: a straight line of standing positions at 1.6 m pitch along the room's inner partition, 0.8 m off the wall, every robot still pickable; room label gains '+N' (design proposal, needs Shaun's approval before build)"],
