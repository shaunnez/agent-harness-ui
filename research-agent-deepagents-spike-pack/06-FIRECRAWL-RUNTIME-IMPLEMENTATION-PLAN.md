...
Planned, not implemented. This is the next bounded vertical slice after the search, capture, and difficult-PDF
benchmarks recorded in `04-SEARCH-PROVIDER-BENCHMARK.md` and `05-CAPTURE-AND-PDF-BENCHMARK.md`.
...
- Firecrawl matched the checked native-text PDF evidence and materially outperformed the current PlanCheck
  path on the degraded scanned plan, although its OCR was not perfect.
...
4. Public HTML and public PDF capture through the existing host-owned `fetch_source` tool.
...
Firecrawl Scrape is the primary capture path for every public URL. Configure it for Markdown main content and
PDF parsing in `auto` mode with:
...
Exit condition: an explicitly selected Deep Agents run uses the new route while default application behavior
is unchanged.
...
1. An explicit `deepagents` run uses Firecrawl for public search and capture when correctly configured.
...
5. Firecrawl captures public HTML and PDFs without the host downloading or storing original PDF bytes.
...
users or tasks implicitly. Start with public PlanCheck research objectives and review provider credit events and
page-grounded citations after each run.
