...
- Keep both selected live-run projections: Mission Map as the orbitable Three.js topology and Signal Loom as the animated handoff view. They are switchable views over the same persisted task, package, candidate, artifact, and gate evidence; neither may invent progress or execution state.
- Hold live-run visual work to a high-definition, source-compared bar: use at least three complete design iterations for substantial fidelity changes, prefer subtle light/material/motion effects over novelty, and make every selectable node reveal truthful context plus relevant evidence and workspace actions.
