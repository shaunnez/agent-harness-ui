...
`prepare-batch.mjs ... feasibility` prepares one balanced-policy H02 trial (`F1`) with the two-hour task allowance, 30M total tokens and one-hour calls for every model stage. These generous allowances now apply to every new preparation mode, with 100 agent-run / 1,000 provider-invocation safeguards. The model matrix and ordinary workflow repair policy remain unchanged. The stage overrides are frozen in the environment and persisted onto the isolated task before dispatch. This is a feasibility probe, not a policy ranking; inspect its result before preparing any repeats.
