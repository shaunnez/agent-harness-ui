...
if (!sourceStat?.isFile()) {
  ...
  try {
    ...
    const jsonGetMs = firstTaskId ? await measureMany(100, () => jsonStore.get(firstTaskId)) : null;
    const jsonUpdateMs = firstTaskId ? await measure(() => jsonStore.update(firstTaskId, () => {})) : null;
    const sqliteStore = new SqliteTaskStore(path.join(directory, "tasks.sqlite3"), {
      legacyJsonPath: copyPath,
    });
    const sqliteInitMs = await measure(() => sqliteStore.init());
    const sqliteListMs = await measureMany(100, () => sqliteStore.listSummaries());
    const sqliteGetMs = firstTaskId ? await measureMany(100, () => sqliteStore.getCore(firstTaskId)) : null;
      ...
      JSON.stringify(
        {
          ...
          timingsMs: {
            ...
            improvementPercent: {
              ...
              update:
                jsonUpdateMs && sqliteUpdateMs != null
                  ? round(100 * (1 - sqliteUpdateMs / jsonUpdateMs))
                  : null,
            },
          },
          payloadBytes: {
            ...
            reductionPercent: round(
              100 * (1 - Buffer.byteLength(summaryPayload) / Buffer.byteLength(fullPayload)),
            ),
