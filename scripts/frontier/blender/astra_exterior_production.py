...
R=Path(__file__).resolve().parents[3]/'design/mission-frontier/assets/staging/cinematic-v1/astra/exterior-production';(R/'renders').mkdir(exist_ok=True);(R/'blend').mkdir(exist_ok=True)
mode='hq'
...
meta={'id':'mf.base.standard.roof','revision':1,'status':'production-pending-final-in-app-review','file':'renders/exterior.png','sourceSize':[1536,1280],'logicalSize':[768,640],'groundAnchor':[384,522],'groundAnchorSource':pixel(anchor),'samples':64,'floorCornersSource':[pixel(v) for v in [(-5,-5,0),(-5,5,0),(5,5,0),(5,-5,0)]],'usedMeshes':sorted(set(used)),'importPlacements':placements,'camera':{'location':list(cam.location),'orthoScale':15.427,'elevationDegrees':30,'azimuthDegrees':45},'shellContactWorldZ':0,'provenance':'Quaternius Modular SciFi FREE STANDARD meshes+trim textures with retained originals and acquisition evidence. Agent-authored terraced drum, segmentation and mechanical dish. Whole exterior shell disappears in HQ.'}
