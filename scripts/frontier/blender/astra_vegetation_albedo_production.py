...
for name,loc,size in [('TwistedTree_3',(.45,-.35,2.75),(5.7,5.75,5.5)),('TwistedTree_5',(-1.85,1.45,1.5),(2.9,2.9,3)),('Bush_Common',(1.1,1.0,.3),(1.2,1.25,.6))]:
 ...
 for slot,m in enumerate(list(o.data.materials)):
  ...
  if 'Leaves' in m.name:
   ...
   tex=nodes.new('ShaderNodeTexNoise');tex.inputs['Scale'].default_value=2.3;tex.inputs['Detail'].default_value=2;coord=nodes.new('ShaderNodeTexCoord');links.new(coord.outputs['Object'],tex.inputs['Vector']);ramp=nodes.new('ShaderNodeValToRGB');ramp.color_ramp.elements[0].color=(.45,.12,.53,1);ramp.color_ramp.elements[1].color=(.82,.4,.75,1);links.new(tex.outputs['Fac'],ramp.inputs[0]);links.new(ramp.outputs['Color'],p.inputs['Base Color']);p.inputs['Roughness'].default_value=.78
...
meta={'id':'mf.prop.purple-tree','revision':2,'status':'production-pending-review','sourceSize':[768,768],'logicalSize':[384,384],'groundAnchor':[192,340],'camera':{'elevationDegrees':30,'azimuthDegrees':45,'location':list(cam.location),'orthoScale':9.5},'samples':64,'usedMeshes':used,'transforms':'TwistedTree3 broad5.7x5.75x5.5, companion TwistedTree5 2.9x2.9x3, lower-trunk taper53percent interpolating to unchanged branching at48percent height. Purple leaf noise coloration and alpha-preserving translucency.','lighting':{'key':[10,-6,16],'keyWatts':4800,'fillWatts':1300,'worldStrength':.55},'buildScriptSha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'provenance':'Quaternius Nature FREE STANDARD CC0; original acquisition/license/archive evidence in sibling sources. No baked ground or shadow.'}
(R/'entry-r2.json').write_text(json.dumps(meta,indent=2)+'\n');bpy.ops.wm.save_as_mainfile(filepath=str(R/'blend/spreading-grove-r2.blend'));S.render.filepath=str(R/'renders/spreading-grove-r2.png');bpy.ops.render.render(write_still=True)


meta['file']='renders/spreading-grove-r2.png';meta['leafAlbedoRange']=[[.45,.12,.53],[.82,.4,.75]];meta['shadow']={'entry':'shadow-entry.json','unchangedGeometryAndLighting':True};(R/'entry-r2.json').write_text(json.dumps(meta,indent=2)+'\n')
