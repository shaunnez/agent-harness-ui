...
R=Path(__file__).resolve().parents[3]/'design/mission-frontier/assets/staging/cinematic-v1/astra/roof-calibration';(R/'renders').mkdir(exist_ok=True);(R/'blend').mkdir(exist_ok=True)
mode='hq'
...
if mode=='hq':
 ...
 path=next((R.parent/'sources/modular-sci-fi-megakit').rglob('WallAstra_Straight.gltf'));used.append(str(path));group='back'
 for side in [0,1]:
 ...
 group='roof'
 chamfered_hull('rounded overhanging eave',(0,0,2.76),(10.05,10.05,.36),white,.75)
 chamfered_hull('deep dark mechanical neck',(0,0,3.1),(8.6,8.6,.55),dark,.9)
 chamfered_hull('substantial rounded upper hull',(0,0,3.58),(8.4,8.4,.7),white,.85)
 chamfered_hull('panoramic blue glass band',(0,0,4.03),(7.7,7.7,.35),blue,.7)
 chamfered_hull('upper ceramic roof plate',(0,0,4.31),(8,8,.3),white,.85)
 for side in [0,1]:
  for i in range(9):
   t=-3.4+i*.85
 ...
 # Connected rooftop service assembly; fittings belong to one physical network.
 chamfered_hull('rear service pod',(-2.9,-2.95,4.77),(1.5,1.55,.62),white,.24)
 cube('service pod inset face',(-2.11,-2.95,4.75),(.065,1.13,.36),dark,.035)
 for yy in [-3.35,-3.08,-2.81,-2.54]:cube('service pod vertical cooling fin',(-2.06,yy,4.75),(.04,.06,.30),metal,.008)
 for xx in [2.35,2.6]:
  cyl('utility vertical riser',(xx,2.8,4.47),(xx,2.8,5.2),.06,metal)
  cyl('connected roof conduit',(xx,2.8,5.2),(xx,-2.5,5.2),.06,metal)
  cyl('conduit descent',(xx,-2.5,5.2),(xx,-2.5,4.48),.06,metal)
 for yy in [-2.4,-1,0.4,1.8]:
  cube('conduit mounting saddle',(2.475,yy,4.83),(.46,.13,.74),dark,.035)
  cyl('saddle crossbar',(2.25,yy,5.19),(2.75,yy,5.19),.035,white)
 for side in [0,1]:
  for t in [-2.4,0,2.4]:
   loc=(4.32,t,3.16) if side==0 else (t,4.32,3.16)
   cube('plenum machinery cassette',loc,(.24,.82,.34) if side==0 else (.82,.24,.34),dark,.04)
   for dt in [-.24,0,.24]:
    q=(4.46,t+dt,3.16) if side==0 else (t+dt,4.46,3.16)
    cube('cassette heat sink',q,(.10,.075,.25) if side==0 else (.075,.10,.25),metal,.015)
 ...
 group='tree';paths=[R.parent/'sources/stylized-nature-megakit/glTF/CommonTree_1.gltf',R.parent/'sources/stylized-nature-megakit/glTF/Bush_Common.gltf'];used=[str(p) for p in paths]
