  ...
  return {
    ...
    actionEligibility: {
      ...
      actions: Object.fromEntries(
        PROJECTED_ACTIONS.map((action) => [action, actionEligibilityFor(task, action)]),
      ),
    },
  };
}
...
function actionEligibilityFor(task, action) {
  ...
  if (action === "reconcile-merge") {
    ...
    return retainedPending || retryableFailure
      ? allow()
      : deny("This task does not have a retained merge intent that can be reconciled.");
  }
  ...
  if (action === "approve-merge") {
    ...
    return stale
      ? deny(`Approval is blocked until ${stale} is fresh for the exact candidate revision.`)
      : allow();
  }
  if (action === "open-pr") {
    if (task.status !== "awaiting-human-approval" || candidate?.status !== "awaiting_human_approval")
    ...
    return stale
      ? deny(`PR approval is blocked until ${stale} is fresh for the exact candidate revision.`)
      : allow();
  }
  if (action === "complete-merged") return task.status === "merged-to-target" ? allow() : deny();
  if (action === "continue-package") {
    const retained = (task.workPackages ?? []).some(
      (workPackage) =>
  ...
  if (action === "retry-test") {
    ...
    const latestTest = [...(task.artifacts ?? [])]
      .reverse()
      .find(
        (artifact) =>
          artifact.stage === "test" &&
    ...
    const candidateDefect = latestTest?.gateResult?.findings?.some(
      (finding) => finding.blocking === true && finding.kind === "candidate-defect",
    );
    return ["repair-required", "failed", "blocked"].includes(task.status) &&
...
export function retryGrantContext(task) {
  ...
  if (stageRuns.some((run) => !terminalStatuses.has(run.status))) {
    return {
      error:
        "The exhausted stage contains a non-terminal run; resolve the inconsistent history before granting a retry.",
    };
  }
  if (new Set(stageRuns.map((run) => run.id)).size !== stageRuns.length) {
    return {
      error:
        "The exhausted stage has duplicate run identities; resolve the inconsistent task state before granting a retry.",
    };
  }
  const reservation = task.stageRunReservations?.[grantedStage] ?? null;
  const candidateBoundGrant =
    ["dev-review", "test", "final-review"].includes(grantedStage) || candidate?.status === "repair_required";
  if (candidateBoundGrant && !validRetryCandidate(candidate)) {
    return {
      error:
        "The exhausted candidate-bound stage is missing an exact candidate identity; resolve it before granting a retry.",
    };
  }
  if (
    !reservation ||
    ...
    !validRetryReservationCandidateBinding(reservation, candidateBoundGrant, candidate, grantedStage, task) ||
    !validRetryReservationKind(grantedStage, reservation.kind)
  ) {
    return {
      error:
        "The exhausted stage has an inconsistent workflow reservation; resolve it before granting a retry.",
    };
  }
  if (!validRetryWorkflowIdentities(stageRuns, reservation, currentAttempts)) {
    return {
      error:
        "The exhausted stage has partial or orphaned workflow identity; resolve it before granting a retry.",
    };
  }
  ...
  if (
    ...
    return {
      error:
        "The exhausted stage run does not match its workflow reservation; resolve the inconsistent history before granting a retry.",
    };
  }
  if (
    exactReservation &&
    stageRuns.some(
      (run) => run.workflowAttempt === currentAttempts && run.workflowReservationId !== exactReservation.id,
    )
  ) {
    return {
      error:
        "The exhausted workflow attempt contains conflicting run reservations; resolve it before granting a retry.",
    };
  }
  ...
  const authorizingGate =
    candidate?.status === "repair_required"
      ...
      : adjacentPriorRevision && !priorTargetRefreshRevision
        ...
        : null;
  if (
    (candidate?.status === "repair_required" || (adjacentPriorRevision && !priorTargetRefreshRevision)) &&
    !authorizingGate
  ) {
    return {
      error:
        "The exhausted candidate repair is missing an exact authorizing gate; resolve the inconsistent history before granting a retry.",
    };
  }
  ...
  const candidateAuthorizerArtifactIds =
    producerEvidence?.authorizerArtifacts.map((artifact) => artifact.id) ?? [];
  const candidateAuthorizerReservationIds =
    producerEvidence?.authorizerReservations.map((entry) => entry.id) ?? [];
...
export function retainQualificationFailuresForImplementationRetry(task, grantedStage) {
  ...
  for (const workPackage of task.workPackages ?? []) {
    const latestVerification = [...(workPackage.verificationRuns ?? [])]
      .reverse()
      .find((verification) => verification.headRevision === workPackage.headRevision);
    if (
      workPackage.status !== "failed" ||
...
export function sameRetryGrantContext(expected, current) {
  ...
  return (
    ...
    JSON.stringify(expected.candidateAuthorizerArtifactIds) ===
      JSON.stringify(current.candidateAuthorizerArtifactIds) &&
    JSON.stringify(expected.candidateAuthorizerReservationIds) ===
      JSON.stringify(current.candidateAuthorizerReservationIds) &&
    JSON.stringify(expected.candidateAuthorizerRunIds) ===
      JSON.stringify(current.candidateAuthorizerRunIds) &&
    JSON.stringify(expected.candidateProducerArtifactIds) ===
      JSON.stringify(current.candidateProducerArtifactIds) &&
