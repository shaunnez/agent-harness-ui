...
import { beginAgentRun, completeAgentRun, refreshGateFreshness } from "./run-activity.mjs";
...
export class WorkPackageOrchestrator extends SpecificationPlanningOrchestrator {
  ...
  async _requalifyRetainedWorkPackage(id, workPackageId, signal) {
    let task = await this._store.get(id);
    const workPackage = task.workPackages.find((item) => item.id === workPackageId);
    ...
    task = await this._store.get(id);
    await this._cleanupSliceWorktree(id, task, retained.worktreePath);
    await this._store.update(id, (draft) => {
      const target = draft.workPackages.find((item) => item.id === workPackageId);
      target.status = "ready_for_integration";
      target.verificationRuns ??= [];
  ...
  async _cleanupSliceWorktree(id, task, worktreePath) {
    try {
