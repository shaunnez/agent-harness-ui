...
const STAGE_PROMPTS = {
  ...
  plan: {
    ...
    instruction:
      "Turn the approved specification and recorded human decisions into a concrete implementation plan. Group work into the smallest coherent work packages that can safely execute in parallel, make dependencies explicit, name repository-relative owned paths (never absolute filesystem paths), and give focused verification commands. Before assigning ownership, inspect the declared verification commands and the package scripts they invoke. For every proposed type, schema or interface change, trace repository references to its constructors, callers, adapters, mocks and preview/test fixtures; include every path that must change to keep that package independently passing. A required field addition needs updates to existing object builders in the same package. A removed or renamed field must remain compatible with callers until they are updated, or those changes must be one package. Include test-registration or configuration paths when new tests require them. Cite the inspected symbols and coupled paths in the Implementation slices section. Prefer fewer coherent packages when shared contracts prevent independent qualification. Ownership is enforced; reading or verifying a file does not grant permission to edit it. Independent packages must not own the same path. Do not implement anything.",
    headings: [
      "Plan summary",
      "Dependency order",
...
function structuredOutputInstruction(stageId, candidate = null, task = null) {
  ...
  if (stageId === "plan") {
    return `\n\nRead .agent-harness/verification.json and reference only command ids it declares. At the end of the Work package manifest section, include exactly one JSON block between <work-packages> and </work-packages> tags. When changes are required, use:\n\n<work-packages>\n{"disposition":"changes-required","evidence":[],"packages":[{"id":"S1","title":"Small outcome","description":"Exact implementation responsibility","dependencies":[],"ownedPaths":["src/example.ts"],"verificationCommandIds":["test"]}]}\n</work-packages>\n\nWhen the requested outcome is already present at the inspected revision, use {"disposition":"already-satisfied","evidence":[{"path":"src/example.ts","detail":"Concrete repository evidence proving the outcome"}],"packages":[]}. When implementation cannot begin because required external evidence, access, credentials, approval, or another prerequisite is unavailable, do not turn an escalation document into a work package. Use {"disposition":"blocked-prerequisite","evidence":[{"path":"scripts/example.ts","detail":"Concrete repository evidence for the dependency"}],"blocker":{"code":"external-data-unavailable","detail":"What is unavailable and why work cannot proceed","requiredAction":"The exact operator or external action needed before planning can resume"},"packages":[]}. A blocked-prerequisite result must not claim the requested delivery is complete. Never infer completion without naming concrete repository path evidence. Use 1-8 packages only for changes-required. IDs must be S1, S2, and so on. Dependencies must reference earlier package IDs and form an acyclic graph. Split only where ownership and verification are genuinely separable. Every package, including documentation-only or configuration-only packages, must contain at least one verificationCommandIds entry from the repository manifest; never emit an empty array or None. If a package description requires adding, updating, or extending tests, ownedPaths must include the exact test file or a test directory; verification permission is not edit ownership. If a proposed package cannot be independently qualified by any declared command, combine it with a package that can. verificationCommandIds must be the smallest focused subset of the repository-owned argv manifest needed to qualify that package.`;
  }
  if (stageId === "test") {
...
function taskProfileStructuredInstruction(candidate) {
  ...
  return `\n\nBecause this task is currently fast, also return exactly one bounded contract. Read .agent-harness/verification.json and reference only ids it declares. A changes-required contract must list at least one of those ids, even for documentation-only or copy-only changes: choose the smallest declared command that still exercises the owned paths, because an empty verificationCommandIds list is rejected and escalates the task out of fast. If any product decision is unresolved, list it; the harness will escalate instead of guessing. ownedPaths must contain exact repository-relative paths, never descriptions such as "the frontend translation resource". Put only positively present risks in riskSignals; do not repeat negated statements such as "no schema changes" there. Use disposition changes-required with one bounded package contract, or already-satisfied with no owned paths or verification commands and concrete path evidence.\n\n<fast-change-contract>\n{"disposition":"changes-required","title":"One coherent change","description":"Exact bounded implementation responsibility","acceptanceCriteria":["Observable outcome"],"evidence":[],"ownedPaths":["src/example.ts"],"verificationCommandIds":["test"],"unresolvedDecisions":[],"riskSignals":[]}\n</fast-change-contract>`;
}
