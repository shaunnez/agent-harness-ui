...
export const CANDIDATE_GATE_COMMAND_LIMIT = 10;
...
export function candidateGateCommandInstruction(stageId) {
  ...
  return limit == null
    ? ""
    : "Use at most ten targeted repository commands for read-only source inspection. Every command must be constructed to exit zero when the intended inspection succeeds: verify file paths before passing them to search tools, and use an explicit no-match allowance such as `rg ... || true` only when finding nothing is a valid result. A non-zero diagnostic command invalidates an otherwise-PASS review and consumes a bounded same-candidate retry; it never proves a candidate defect.";
}
