#!/usr/bin/env python3
"""Minimal MCP stdio server exposing the local priced-rate capture. No dependencies.

Ported verbatim from `research-agent-deepagents-spike-pack/14a-qv-mcp-server.py`, which is the
tool server behind all 90 recorded runs, with one change: the capture path comes from argv or
`RESEARCH_QV_INDEX` instead of being hard-coded to one developer's home directory.

Read `nearest_group` before changing anything here. It carries the in-table sub-heading — load
class, size band, duty — and omitting it from tool output made the agent infer load classes
from the price ladder. Including it dropped `not_established` from 8 entries to 5 on one
scenario. It is in `fmt()` for that reason and must stay there.
"""
import json, sys, re, math, collections, os

IDX = (sys.argv[1] if len(sys.argv) > 1 else None) or os.environ.get("RESEARCH_QV_INDEX") or ""
if not IDX:
    sys.exit("Pass the capture path as argv[1] or set RESEARCH_QV_INDEX.")
if not os.path.exists(IDX):
    sys.exit(f"Priced-rate capture not found at {IDX}.")
