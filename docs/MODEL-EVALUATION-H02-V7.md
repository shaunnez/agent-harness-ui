...
- Task: unchanged H02 v2 public contract and historical base `4b303a8da4fafd7a78149ddad40cbf714411d08b`. Independent h02-v4 grader has eleven checks. Using current main as the task base would change the case.
...
Private fresh campaign: `/Users/shaun/.codex/model-evaluation/20260922/feasibility-v7b`. Public isolated candidate root: `/private/tmp/h-eval-f7b`. `CURRENT.md` in the private evidence parent records the latest status. No delivery result is claimed by this preparation record.

Preparation caught two zero-inference apparatus issues before delivery: the task API's old 100M validation cap rejected the authorized 200M allowance, and the grader's Vite `port: 0` fell through to busy-port fallback at 5175, which the historical API correctly rejects. The validator now admits 200M. Grader v4 binds strictly to the historical allowed preview origin on 4173, preserving the API boundary and all eleven assertions. Original failed preparation/control receipts remain in `feasibility-v7` and `preparation-v7/grader-controls`; newly frozen execution uses `feasibility-v7b`. No candidate code was changed and no model was called by either failure.
