...
- Development Review may use at most ten targeted repository commands for diff/source inspection and is explicitly forbidden from running tests, builds, linters, type checks, package scripts, or manifest commands. The runtime stops a review that attempts an eleventh command.
...
5. Review gates require structured candidate-bound evidence. Each finding is classified as `candidate-defect` or `verification-gap`. Reviewer tooling failures create `review_retry_required`; they override even a narrative REPAIR verdict and never authorize candidate Repair. The same reason receives one normal retry, then exhausts the stage allowance until a human grants another attempt. Verification gaps stay non-blocking for Dev Review because the later Harness-owned Test gate resolves them. P2/P3 candidate advice is retained as non-blocking follow-up unless tied to an acceptance criterion.
