...
Tasks may opt into a governed design exploration after Grill and before Task Spec. This is a subflow inside **Task Spec**, not an eleventh stage: Claude Design and Codex Design run independently, retain two provider-attributed prototype revisions, and pause for an explicit operator selection. Both generators receive the task brief, authoritative decisions, and bounded retained Triage, Scout, and Grill evidence; they do not receive direct repository access. Their prompts remain task-driven and must preserve the existing product anatomy and behaviour when the requested change is a refinement rather than a redesign. Task Spec cannot start until one complete revision is selected. The selected revision is immutable for that specification run and is supplied—with its URL or local bundle hash, summary, and context manifest—to Specification, Plan, implementation, review, and test. The rejected variant remains inspectable for provenance but is never blended into downstream context. Provider failure retains partial output but fails the subflow closed; the operator may retry both generators. Local prototype HTML is served from a sandboxed frame under a restrictive content-security policy, and server filesystem paths are never projected to the client.

Design generation owns a separate provider-specific policy pair because both generators run concurrently. Settings default Claude Design to **Claude Opus 5.5 / High** and Codex Design to **GPT-5.6 Sol / High**. New Task may override either with an eligible model and supported reasoning level from that provider. Creation snapshots both exact policies into the design request and every variant records its policy before dispatch; Settings changes cannot rewrite the task, retries reuse the same snapshot, and provider/model drift fails with an actionable error instead of substituting another model. Legacy unstarted requests retain the former Sonnet/Luna fixed defaults with explicit migration provenance, while completed variants retain the models they actually recorded.
...
Frontier Settings exposes a Repair manual/automatic choice and numeric repair allowances. New tasks snapshot the allowances: two additional automatic corrections per package, and one/two/three candidate repairs for fast/standard/high-risk by default. Each value accepts 0–10; zero disables that category. The candidate allowance is shared across Dev Review, Test and Final Review, counts failed and cancelled repair calls as attempts, and also bounds manually requested candidate repairs. A further defect after exhaustion stops for human direction. Every changed candidate creates a new immutable revision, makes downstream evidence stale, and reruns every invalidated gate. Settings changes do not reset an existing task's counters or limits; gate mode changes apply at the next gate.

Before integration, an ordinary package check failure may return automatically to the implementation agent in the exact retained worktree. Supply all failed command details, retain each run and check, link successive corrections explicitly, and requalify the corrected commit before assembly. Preserve dependency commits and ownership boundaries. Provider failures, cancellation, invalid plans, baseline failures, command-start failures and timeouts do not trigger this code-repair loop. The existing fast-profile replan/escalation rule still applies to fast qualification failures. Package corrections use the task's snapshotted Implement policy; candidate repairs use its Repair policy. No model substitution or reasoning escalation is implicit.
