...
test("does not reserve or publish when repository authority drifts before PR reservation", async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "agent-harness-authority-race-approval-"));
  try {
    const store = new JsonTaskStore(path.join(directory, "tasks.json"));
    await store.init();
    const task = await createApprovalReadyTask(store, directory, "Reject authority drift");
        ...
        if (property === "transition") {
          return async (...args) => {
            if (!transitioned) {
              transitioned = true;
              await target.update(task.id, (draft) => {
                draft.repositoryAuthorityStatus = "legacy-unbound";
              });
            }
            return target.transition(...args);
          };
    ...
    await assert.rejects(
      ...
      (error) =>
        ...
        /repository authority/i.test(error.message) &&
        error.evidence.some((item) => /not bound/i.test(item)),
    );
    const rejected = await store.get(task.id);
    assert.equal(publishCalled, false);
    assert.equal(rejected.status, "awaiting-human-approval");
