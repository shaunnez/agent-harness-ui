...
} from "../scripts/evaluation/medium-provider-comparison.mjs";
import {
...
test("H02 compares Sol and Sonnet on the qualified high-risk task", async () => {
  const selected = await loadEvaluationCase("H02");
  assert.equal(selected.workflowProfile, "high-risk");
  assert.equal(selected.checkIds.length, 11);
  const { policies, trials, allowedModels, allowedProviders, trialConcurrency } =
    H02_MEDIUM_PROVIDER_COMPARISON;
