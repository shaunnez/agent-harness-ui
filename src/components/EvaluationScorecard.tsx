        ...
        <span>
          ...
          <p>
            {experiments?.methodology ??
              "Frozen task variants appear here after they are explicitly attached to an experiment."}{" "}
            {experiments?.taskCount ?? 0} task samples.
          </p>
        </span>
      </div>
      <div className="evaluation-table">
        ...
        {(experiments?.variants ?? []).map((variant) => (
          <div className="evaluation-table__row" key={`${variant.groupId}:${variant.variantId}`}>
            ...
            <code>
              {variant.repairCount} / {variant.retryCount} retry
            </code>
            <code>{quality(variant)}</code>
          </div>
        ))}
        {!experiments?.variants?.length ? (
          <div className="evaluation-table__empty">
            No controlled samples yet. Historical runs remain below and are never promoted into experiments
            automatically.
          </div>
        ) : null}
      </div>
        ...
        <details className="evaluation-evidence" key={`evidence:${variant.groupId}:${variant.variantId}`}>
          <summary>
            <span>
              ...
              <small>Measurement evidence and confound checks</small>
            </span>
            <span>
              {variant.sampleCount} samples <CaretDown size={15} />
            </span>
          </summary>
          <div className="evaluation-evidence__metrics">
            <span>
              <small>Avg. wall time</small>
              <strong>{duration(variant.averageWallTimeMs)}</strong>
            </span>
            <span>
              <small>Input / output</small>
              <strong>
                {variant.inputTokens.toLocaleString()} / {variant.outputTokens.toLocaleString()}
              </strong>
            </span>
            <span>
              <small>Cache rate</small>
              <strong>
                {variant.cacheRate == null ? "Unavailable" : `${Math.round(variant.cacheRate * 100)}%`}
              </strong>
            </span>
            <span>
              <small>Work credits</small>
              <strong>{variant.credits == null ? "Unavailable" : variant.credits.toFixed(2)}</strong>
            </span>
            <span>
              <small>API-rate estimate</small>
              <strong>
                {variant.apiEstimate == null ? "Unavailable" : formatApproximateCost(variant.apiEstimate)}
              </strong>
            </span>
            <span>
        ...
        <span>
          ...
          <p>
            {observations?.methodology ??
              "Observed retained model runs grouped by exact role, model, and reasoning."}{" "}
            Only roles that actually ran appear; deterministic handoffs and unrun skills are omitted. Quality
            is shown only after an operator records a task evaluation.{" "}
            {observations?.evaluatedTasks ?? summary?.evaluatedTasks ?? 0} tasks currently have ratings.
          </p>
        </span>
      </div>
      <div className="evaluation-table">
        ...
        {(observations?.variants ?? summary?.variants ?? []).map((variant) => (
          <div
            ...
            key={`${variant.role}:${variant.model}:${variant.reasoning}`}
          >
            <span>
              <strong>{variant.role}</strong>
              <small>
                {variant.model} · {variant.reasoning}
              </small>
            </span>
            ...
            <code>
              {variant.averageHumanScore == null
                ? "No task rating"
                : `${variant.averageHumanScore.toFixed(1)} / 5`}
            </code>
          </div>
        ))}
        {!(observations?.variants ?? summary?.variants)?.length ? (
          <div className="evaluation-table__empty">
            No observed agent runs yet. The scorecard uses retained runtime evidence and does not invent
            success rates.
          </div>
        ) : null}
      </div>
