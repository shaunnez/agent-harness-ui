...
import { formatTaskDate, type RuntimeChangelogCommit, type RuntimeChangelogDetail, type RuntimeChangelogDiff } from "../domain";
...
export function ChangelogModal({ onClose }: { onClose: () => void }) {
  ...
  const selectCommit = async (commit: RuntimeChangelogCommit) => {
    setError(null);
    setDetailLoading(true);
    setFileDiff(null);
    try {
      setSelected(await getChangelogCommit(commit.sha));
    } catch (reason) {
  ...
  const selectFile = async (filePath: string) => {
    ...
    setDetailLoading(true);
    try {
      setFileDiff(await getChangelogFileDiff(selected.sha, filePath));
    } catch (reason) {
      ...
      <section className="changelog-modal">
        ...
        {fileDiff ? (
          <div className="changelog-diff">
            <div className="changelog-detail__bar"><Button tone="ghost" compact icon={ArrowLeft} onClick={() => setFileDiff(null)}>Back to commit</Button><span><strong>{fileDiff.path}</strong><small>{fileDiff.sha.slice(0, 8)}{fileDiff.truncated ? " · truncated" : ""}</small></span></div>
          ...
          <div className="changelog-layout">
            <aside className="changelog-list">
              ...
              {commits.map((commit) => <button type="button" key={commit.sha} className={selected?.sha === commit.sha ? "is-selected" : ""} onClick={() => void selectCommit(commit)}><GitCommit size={16} /><span><strong>{commit.subject}</strong><small><code>{commit.shortSha}</code> · {commit.author} · {formatTaskDate(commit.authoredAt)}</small></span></button>)}
            ...
            <main className="changelog-detail">
              {detailLoading ? <ChangelogDetailSkeleton /> : selected ? (
                <>
                  ...
                  <div className="changelog-files"><strong>{selected.files.length} changed files</strong>{selected.files.map((file) => <button type="button" key={`${file.status}:${file.path}`} onClick={() => void selectFile(file.path)}><span className="changelog-file-status">{file.status}</span><FileCode size={16} /><span><strong>{file.path}</strong>{file.previousPath ? <small>from {file.previousPath}</small> : null}</span></button>)}</div>
                </>
