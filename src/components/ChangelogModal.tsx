...
export function ChangelogModal({
  ...
  useEffect(() => {
    ...
    void listChangelog()
      ...
      .catch((reason) => {
        if (current)
          setError(reason instanceof Error ? reason.message : "Live git history could not be loaded.");
      })
      .finally(() => {
  ...
  return (
    <div className="changelog-overlay" role="dialog" aria-modal="true" aria-label="Repository changelog">
      <button
        type="button"
        className="changelog-overlay__backdrop"
      ...
      <section className="changelog-modal">
        <header>
          <span>
            <GitCommit size={20} />
            <span>
              <small>Live git history · read-only</small>
              <strong>Repository changelog</strong>
            </span>
          </span>
          <button
            ref={closeRef}
        ...
        {fileDiff ? (
          <div className="changelog-diff">
            <div className="changelog-detail__bar">
              <Button
                tone="ghost"
                compact
                icon={ArrowLeft}
                onClick={() => onSelectCommit(commitSha ?? fileDiff.sha)}
              >
                Back to commit
              </Button>
              <span>
                ...
                <small>
                  {fileDiff.sha.slice(0, 8)}
                  {fileDiff.truncated ? " · truncated" : ""}
                </small>
              </span>
            </div>
            {fileDiff.diff.trim() ? (
              <UnifiedDiff diff={fileDiff.diff} />
            ) : (
              <div className="changelog-empty">Git returned no textual diff for this file.</div>
            )}
          </div>
        ) : (
          <div className="changelog-layout">
            <aside className="changelog-list">
              <div>
                <strong>Last 10 commits</strong>
                <small>{loading ? "Loading live history…" : `${commits.length} commits`}</small>
              </div>
              {commits.map((commit) => (
                <button
                  type="button"
                  key={commit.sha}
                  className={selected?.sha === commit.sha ? "is-selected" : ""}
                ...
                >
                  ...
                  <span>
                    ...
                    <small>
                      <code>{commit.shortSha}</code> · {commit.author} · {formatTaskDate(commit.authoredAt)}
                    </small>
                  </span>
                </button>
              ))}
              {!loading && !commits.length ? (
                <div className="changelog-empty">No live commits were returned.</div>
              ) : null}
            </aside>
              ...
              {detailLoading ? (
                ...
                <>
                  <header>
                    <span>
                      <small>
                        {selected.shortSha} · {formatTaskDate(selected.authoredAt)}
                      </small>
                  ...
                  <div className="changelog-files">
                    <strong>{selected.files.length} changed files</strong>
                    {selected.files.map((file) => (
                      ...
                      >
                        <span className="changelog-file-status">{file.status}</span>
                        <FileCode size={16} />
                        <span>
                ...
                <div className="changelog-empty changelog-empty--center">
                  <GitCommit size={28} />
                  <strong>Select a commit</strong>
                  <span>Inspect its metadata, changed files, and file-level diff.</span>
                </div>
              )}
            </main>
          </div>
...
function ChangelogDetailSkeleton() {
  return (
    <div className="changelog-skeleton" role="status" aria-label="Loading commit details">
      <span />
      <span />
      <span />
      <span />
