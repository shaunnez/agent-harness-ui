...
export function stageUsage(tasks: RuntimeTaskSummary[], stageId: AgentRoleId) {
  const artifacts =
    stageId === "scouts" || stageId.startsWith("scout-")
      ? tasks.flatMap((task) => {
      ...
      : tasks
          ...
          .filter(
            (artifact) => isModelRunArtifact(artifact) && (artifact.agentRole ?? artifact.stage) === stageId,
          );
...
export function UsageSummary({ tasks, roleId }: { tasks: RuntimeTaskSummary[]; roleId: AgentRoleId }) {
  ...
  return (
    <div className="detail-metrics detail-metrics--truthful">
      ...
      <Metric
        label="Approx. cost"
        value={usage.pricedRuns ? formatApproximateCost(usage.cost) : "Unavailable"}
      />
      <Metric
        label="Work credits"
        value={usage.creditRuns ? (usage.credits?.toFixed(3) ?? "Unavailable") : "Unavailable for provider"}
      />
    </div>
  );
}
