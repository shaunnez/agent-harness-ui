...
export function AgentPolicyEditor({
  ...
  const isGlobalDefault = Boolean(
    globalDefault && value.model === globalDefault.model && value.reasoning === globalDefault.reasoning,
  );
  ...
  return (
    <div className="agent-policy-editor">
      ...
      <label htmlFor={`${idPrefix}-reasoning`}>
        ...
        >
          {(selectedModel?.reasoningLevels ?? [value.reasoning]).map((level) => (
      ...
      {onReset ? (
        <Button
          ...
          title={
            isGlobalDefault
              ? "This policy already matches the global default."
              : "Stage the global default in this editor."
          }
        >
