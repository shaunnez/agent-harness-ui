...
import { getFreshnessLabel, getFreshnessMessage } from "./workflow";
      ...
      {inventory.map((row) => (
        ...
        >
          ...
          <span className="runtime-worktree-row__badges">
            ...
            <span className={`badge badge--${row.cleanupReady ? "green" : "yellow"}`}>
...
export function RuntimeFocusedTestEvidencePanel({
  ...
  const evidenceFreshness = freshness === undefined ? evidence.freshness : freshness;
  const evidenceIsFresh = evidenceFreshness?.state === "fresh";
  const selectedRow = evidence.rows.find((row) => row.id === selectedResultId) ?? null;
  const passed = evidence.rows.filter(
    (row) => row.freshness?.state === "fresh" && row.status === "passed",
  ).length;
  const failed = evidence.rows.filter(
    (row) => row.freshness?.state === "fresh" && row.status === "failed",
  ).length;
  const staleRows = evidence.rows.length - passed - failed;
  const selectedRowIsFresh = selectedRow?.freshness?.state === "fresh";
  useEffect(() => {
  ...
  return (
    <section
      className={`runtime-focused-test ${evidenceIsFresh ? "" : "runtime-focused-test--stale"}`}
      aria-label="Focused test evidence"
    >
      <header>
        <span>
          <small>
            {evidenceIsFresh ? "Candidate-bound structured evidence" : "Superseded candidate-bound evidence"}
          </small>
          <strong>
            {passed} passed &middot; {failed} failed{staleRows ? ` · ${staleRows} stale` : ""} &middot;{" "}
      ...
      {!evidenceIsFresh ? (
        <div className="runtime-stale-banner" role="status">
          <strong>{getFreshnessLabel(evidenceFreshness)}</strong> &middot;{" "}
          {getFreshnessMessage(evidenceFreshness)} Stored results remain available for audit, but cannot count
      ...
      {selectedRow ? (
        <div className="runtime-focused-test__detail">
          ...
          <div className="runtime-focused-test__detail-title">
            {selectedRowIsFresh && selectedRow.status === "passed" ? (
            ...
            <span>
              <small>
                {selectedRowIsFresh ? `${selectedRow.status} result details` : "Superseded result details"}
              </small>
        ...
        <div className="runtime-focused-test__rows">
          {evidence.rows.map((row) => (
            <button type="button" key={row.id} onClick={() => onSelectResult(row.id)}>
              {row.freshness?.state === "fresh" && row.status === "passed" ? (
                <CheckCircle size={18} weight="fill" />
              ) : (
                <WarningCircle size={18} weight="fill" />
              )}
              ...
              <span>
                ...
                >
                  {row.freshness?.state === "fresh" ? row.status : "rerun required"}
                </strong>
      ...
      <footer>
        ...
        <small>
          {evidenceIsFresh
            ...
            : "Rerun required before this Test evidence can be used"}
        </small>
      </footer>
    </section>
  );
