...
export function overlayHash(overlay: Overlay) {
  ...
  if (overlay.kind === "research" || overlay.kind === "research-ask") {
    parts.push(overlay.projectId);
    if ("questionId" in overlay && overlay.questionId) parts.push(overlay.questionId);
...
export function parseOverlay(hash: string): Overlay | null {
  try {
    ...
    if (kind === "research" && id) return { kind, projectId: id, questionId: detail || undefined };
    if (kind === "research-ask" && id) return { kind, projectId: id };
